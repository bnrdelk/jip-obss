...
const SellerRegisterForm = () => {
    ...
    const handleSubmit = async (values) => {
        try {
            await axiosInstance.post('/auth/register/seller', values);
            setMessage('Seller registered successfully');
        } catch (error) {
            // Extract error message from the response if available
            const errorMessage = error.response?.data?.message || 'Failed to register seller';
        ...
        <Row justify="center" align="middle" style={{ height: '100vh' }}>
            <Col xs={24} sm={18} md={12} lg={8}>
                <div style={{ padding: '24px', background: '#fff', borderRadius: '16px', boxShadow: '0 8px 8px rgba(0, 0, 0, 0.1)', backgroundColor: "rgba(91, 114, 150, 0.39)"}}>
                    ...
                    <Title level={2} style={{ textAlign: 'center' }}>Register as Seller</Title>
